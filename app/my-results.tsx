...
import { loadMyResultCards, type MyCardItem } from '@/src/features/my/loadMyCards';
...
export default function MyResultsScreen() {
  ...
  const load = useCallback(async () => {
    setError(null);
    const res = await loadMyResultCards();
    if (res.error) {
      setItems([]);
  ...
  if (loading) {
    return (
      <View style={styles.center}>
        <Stack.Screen options={{ title: t('my.myResultsTitle') }} />
        <ActivityIndicator />
      </View>
    );
  }
    ...
    <ScrollView
      ...
      {!items.length ? (
        <View style={styles.card}>
          <Text style={styles.title}>{t('my.emptyMyResults')}</Text>
          <Pressable style={styles.button} onPress={() => router.replace('/(tabs)' as any)}>
            <Text style={styles.buttonText}>{t('my.goTodayMission')}</Text>
          </Pressable>
        </View>
      ) : (
